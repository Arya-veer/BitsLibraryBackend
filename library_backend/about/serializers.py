...
class LibraryCommitteeSerializer(serializers.ModelSerializer):
    ...
    def get_members(self,obj):
        return CommitteeMemberSerializer(obj.members.filter(is_present = True).order_by('position'),many = True,context = self.context).data
    ...
    def get_holiday_reason(self,obj):
        if self.get_is_currently_open(obj) is None:
            ...
            if self.current_time < obj.opening_time:
                return f"The library is temporarily closed and is scheduled to resume services at {self.get_opening_time(obj)}"
            else:
